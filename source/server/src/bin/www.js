...
const START_SERVER = () => {
  ...
  io.on("connection", (socket) => {
    myLogger.info(`New client connected: ${socket.id}`);
    socket.on("disconnect", () => {
      myLogger.info(`Client disconnected: ${socket.id}`);
    });
  });
...
(async () => {
  try {
    ...
    myLogger.info("2.1. Seeding data...");
    await seedAdmin();

    myLogger.info("3. Connecting to MQTT Broker...");
    mqttService.connect();

    myLogger.info("4. Starting server...");
    START_SERVER();
  } catch (error) {
